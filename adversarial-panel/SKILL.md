...
description: >-
  Run a multi-model adversarial mutual review with facilitator synthesis:
  2+ panelists (different model families where possible — use codex-server /
  codex-cli for the external-family seat) independently answer, adversarially
  critique each other across rounds, and the main session acts as facilitator,
  synthesizing agreements, live disagreements, and calibrated confidence into
  one answer. Use ONLY on high-stakes, hard-to-reverse claims, designs or
  decisions, and only when the user EXPLICITLY asks to debate or adversarially
  review with multiple models — "discuss with Opus and Codex", "red-team
  this", "devil's advocate", 敵対的レビュー, 相互レビュー, 多モデル討論, 議論させて.
  A full run costs ~6 subagent/CLI dispatches: state the panel size and round
  count and get a go-ahead before launching. Where the Workflow tool is
  available, run the panel as a Workflow (judge-panel / adversarial-verify)
  instead of ad-hoc fan-out. Do NOT self-trigger on a bare "are you sure?" —
  that is a cue to OFFER the panel, not to run it.
---
...
## Local interop notes (this install, not upstream)

Everything above this section is upstream verbatim. These notes adapt it to
this machine; they take precedence where they conflict.

- **External-family seat.** Tier 1 ("different model families") resolves to the
  `codex-server` skill, or `codex-cli` for one-shot batch turns. Both use the
  ChatGPT subscription (`codex login status` → "Logged in using ChatGPT"), not
  `OPENAI_API_KEY` billing. Prefer `codex-server`: its turns return a turn-id
  immediately and stream to files, so a long panelist never has to block a
  foreground Bash call. Do not shell out to a bare `codex` binary.
- **Bash timeout.** If you do run a foreground CLI panelist, set the Bash
  `timeout` explicitly — the default is 120000 ms and the maximum is 600000 ms
  (10 min). Upstream's "generous timeout, e.g. 10 min" is exactly this maximum.
- **Prefer the Workflow tool.** When the Workflow tool is available, express
  the panel as a Workflow (its documented judge-panel and adversarial-verify
  patterns) rather than ad-hoc `Agent` fan-out: schema-validated returns make
  the Round-1 validation gate mechanical instead of a thing you must remember.
- **Cost gate.** Default 2 panelists × 3 rounds ≈ 6 dispatches. Say the panel
  size and round count, and get a go-ahead, before launching.
- **Adjacent skills.** `prd-council` is document-generation for PRDs only;
  `review-local` reviews a diff; `parallel-investigate` has no rebuttal round.
  Use this skill when the subject is a general claim/design/decision.
