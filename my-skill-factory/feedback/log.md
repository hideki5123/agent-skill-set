...
- **User Note**: —
- **Proposed amendment (1)**: add a **pre-install CLI-flag verification** step alongside the existing path-discipline grep — for every shell command in the skill source, run the real `<cmd> --help` and confirm each subcommand and flag exists. Cheap, mechanical, and it would have caught 6 of the 6 critical bugs before install.
- **Second finding — the install pipeline itself was broken.** Step 5a (the `claude -p` listing check) failed for this skill, and chasing it down uncovered a latent bug in `install_skill.py` / `scripts/skill_layout.py`: they wrote `"agents": "./agents"` into `plugin.json` for any skill shipping an `agents/` dir. Claude Code rejects that manifest (`Plugin X has an invalid manifest file`) — **and an invalid manifest silently unregisters the plugin's SKILLS too.** Nine skills were installed, enabled, catalogued, and invisible to every session: `dotnet-senior`, `naturalize-ja`, `orch-qa`, `pm-review`, `postmortem`, `pr-review`, `review-local`, `skill-improve`, `subagent-gen`. `skill_layout.py`'s validator *required* the key, so a drift check would have re-added it. Fixed in `e973f8a`; agents are discovered from `agents/` by convention, as the official plugins do.
- **Proposed amendment (2)**: Step 5a must be treated as **blocking**, and its failure must not be explained away. The listing check DID catch this, but the obvious excuse ("it's a worktree; the marketplace resolves to the main checkout") was true *and* not the whole cause. Add to Step 5: on a listing miss, run `claude plugin list | grep -i "invalid manifest"` and `claude plugin details <name>` before attributing the miss to environment.
- **Proposed amendment (3)**: never bulk re-run `install_skill.py` to repair plugins — it defaults `version` to 1.0.0 and silently downgrades any skill installed with `--version` (it downgraded `review-local` 2.0.0 → 1.0.0 mid-session; caught and reverted). Repair manifests surgically.
---
