...
Only get comments made on a specific commit (useful for tracking which comments are new after a push):

```bash
gh api repos/{owner}/{repo}/pulls/{number}/comments --paginate \
  | jq --arg sha "$COMMIT_SHA" '[.[] | select(.commit_id == $sha)]'
```

### Exclude already-processed comment IDs

```bash
gh api repos/{owner}/{repo}/pulls/{number}/comments --paginate \
  | jq --argjson seen '[111, 222, 333]' \
...
## Re-requesting Reviews After a Push

After pushing fixes and wanting a fresh review, re-request the reviewers. Some reviewers require removing the old request first:

### Simple re-request (works for most cases)
...
### Force re-request (DELETE then POST)

If a reviewer has already submitted a review and the simple re-request doesn't trigger a new review:

```bash
# Remove the reviewer first
gh api --method DELETE /repos/{owner}/{repo}/pulls/{number}/requested_reviewers \
  -f "reviewers[]=copilot-pull-request-reviewer[bot]"
...
### Skip re-requesting if already reviewed current HEAD
...
if [ "$REVIEWED" -gt 0 ]; then
  echo "Copilot already reviewed this commit, skipping re-request."
fi
```
...
Verify the PR is still open before each loop iteration:

```bash
gh pr view <number> --json state --jq '.state'
```

Returns `OPEN`, `CLOSED`, or `MERGED`. Only continue the loop if `OPEN`.

Full state check with additional context:

```bash
gh pr view <number> --json state,isDraft,mergeable --jq '{state, isDraft, mergeable}'
```
