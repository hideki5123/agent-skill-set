...
name: confluence
description: >
  Use confluence-cli to read, search, create, update, move, and delete
  Confluence pages and attachments from the terminal.
  Prefer this skill over `mcp__claude_ai_Atlassian__*` MCP tools when you need:
  page export with attachments, copy-tree, attachment management, profile-based
  multi-instance access, read-only mode, CQL search, or shell pipelines.
  Trigger phrases include "confluence", "confluence-cli", "confluence page",
...
- Always use `confluence` CLI — it must be globally installed via npm.
- Configuration must already be set up (env vars or `confluence init`). If not, guide the user through setup.
- For simple Confluence read/create/update operations where `mcp__claude_ai_Atlassian__*` MCP tools suffice, prefer MCP (less overhead, no shell spawning). Switch to CLI for features MCP lacks.

## Preflight Check

Before running any confluence command, verify the installation:
...
If this fails:
- **"command not found"** — confluence-cli is not installed.
  Suggest: `npm install -g confluence-cli`
- **"No configuration found"** — Config not set up.
  Suggest: `confluence init` or set env vars (`CONFLUENCE_DOMAIN`, `CONFLUENCE_API_TOKEN`)

## When to Use CLI vs. MCP

| Capability | `mcp__claude_ai_Atlassian__*` MCP | `confluence` CLI |
|------------|-----------------------------------|------------------|
| Read page | Yes — `getConfluencePage` | Yes — `confluence read` |
| Search (CQL) | Yes — `searchConfluenceUsingCql` | Yes — `confluence search` |
| Create page | Yes — `createConfluencePage` | Yes — `confluence create` |
| Update page | Yes — `updateConfluencePage` | Yes — `confluence update` |
| Get comments | Yes — various comment tools | Yes — `confluence comments` |
| Add comment | Yes — `createConfluenceFooterComment` | Yes — `confluence comment` |
| List spaces | Yes — `getConfluenceSpaces` | Yes — `confluence spaces` |
| Page descendants | Yes — `getConfluencePageDescendants` | Yes — `confluence children` |
| **Export page + attachments** | No | Yes — `confluence export` |
| **Copy page tree** | No | Yes — `confluence copy-tree` |
| **Move page** | No | Yes — `confluence move` |
| **Delete page** | No | Yes — `confluence delete` |
| **Attachment upload/download** | No | Yes — `confluence attachment-upload` / `attachments --download` |
| **Attachment delete** | No | Yes — `confluence attachment-delete` |
| **Edit raw storage XML** | No | Yes — `confluence edit` |
| **Profile-based multi-instance** | No | Yes — `--profile` flag |
| **Read-only mode** | No | Yes — `CONFLUENCE_READ_ONLY=true` |
| **Find page by title** | No | Yes — `confluence find` |
| **Page info/metadata** | No | Yes — `confluence info` |

**Rule of thumb:** Use MCP for simple read/create/update. Use CLI for export, copy-tree, attachments, move, delete, multi-instance, and read-only mode.

## Quick Reference

| Action | Command |
|--------|---------|
| Read page | `confluence read <pageId> --format markdown` |
| Page metadata | `confluence info <pageId>` |
| Find by title | `confluence find "Title" --space SPACE` |
| Search | `confluence search "query" --limit 20` |
| List spaces | `confluence spaces` |
| List children | `confluence children <pageId> --recursive --format tree` |
| Create page | `confluence create "Title" SPACE --file ./content.md --format markdown` |
| Create child | `confluence create-child "Title" <parentId> --content "# Hello" --format markdown` |
| Update page | `confluence update <pageId> --file ./updated.md --format markdown` |
| Move page | `confluence move <pageId> <newParentId>` |
| Delete page | `confluence delete <pageId> --yes` |
| Export page | `confluence export <pageId> --format markdown --dest ./docs` |
| Copy tree | `confluence copy-tree <sourceId> <targetParentId> --dry-run` |
| List attachments | `confluence attachments <pageId>` |
| Upload attachment | `confluence attachment-upload <pageId> --file ./report.pdf` |
| Download attachments | `confluence attachments <pageId> --download --dest ./downloads` |
| List comments | `confluence comments <pageId> --format json --all` |
| Add comment | `confluence comment <pageId> --content "Note" --location footer` |
| Switch profile | `confluence profile use staging` |
...
### Scenario 1: Read a Confluence page

```gherkin
Given confluence-cli is installed and configured
When the user asks to read a Confluence page (by ID or URL)
Then run `confluence read <pageId> --format markdown`
And present the page content
```

### Scenario 2: Search Confluence

```gherkin
Given confluence-cli is installed and configured
When the user asks to search Confluence for a term
Then run `confluence search "<term>" --limit 10`
And present matching pages with titles and IDs
```

### Scenario 3: Create or update a page

```gherkin
Given confluence-cli is installed and configured
When the user asks to create or update a Confluence page
Then prefer `--format markdown` for agent-generated content
And use `--file` for larger content (write to a temp file first)
And report the page ID and URL on success
```

### Scenario 4: Export a page tree

```gherkin
Given confluence-cli is installed and configured
When the user asks to export a Confluence page
Then run `confluence export <pageId> --format markdown --dest <directory>`
And report the exported files and attachment count
```

### Scenario 5: confluence-cli not installed

```gherkin
Given confluence-cli is not installed or not configured
When the user asks to perform any Confluence CLI operation
Then check with `confluence --version`
And guide the user to install: `npm install -g confluence-cli`
And guide config setup: `confluence init` or set env vars
```

### Scenario 6: Multi-instance operation

```gherkin
Given the user works with multiple Confluence instances
When the user specifies an instance or profile
Then use the `--profile <name>` flag on the command
And if no profile exists, guide setup with `confluence profile add <name>`
```
