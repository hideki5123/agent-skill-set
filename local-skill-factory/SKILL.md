...
3. If the skill needs project-wide persistent instructions (rare), suggest updating CLAUDE.md

4. Confirm: "Created `/project:<name>` — try it with `/<name>` in this project."
...
3. If the user provides feedback OR if corrections/issues occurred:
   a. Create `feedback/` directory in the skill source (`D:\Shared\agents\my-skills\local-skill-factory\feedback\`) if it does not exist
